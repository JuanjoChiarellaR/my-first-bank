...
function buildContextLayers(context) {
  ...
  const baseline = {
    ...
    // Only present when every product below shares one date (see
    // js/semantic-layer.js's uniformProductsDate) — the per-product
    // last_verified_date field is omitted from the three all_* arrays in
    // that case, so the agent needs this instead. See SYSTEM_PROMPT's field
    // glossary for how it's told to use whichever shape it gets.
    ...(MFB.semantic.productsLastVerifiedDate ? { products_last_verified_date: MFB.semantic.productsLastVerifiedDate } : {}),
  };
