...
- **Only informs, never recommends — stated as an absolute boundary, not just a numbered rule.** Never says "you should choose X," ranks institutions, or gives a "do this first, then this" sequenced plan — presents facts side by side and lets the user decide; an objective, explicitly-requested sort (e.g. "sorted by lowest fee") is fine, a subjective "best for you" ranking is not. This is a deliberate product and liability-reduction decision — a hard rule, not a style preference, and it explicitly holds regardless of any user-supplied persona/role ("act as my financial coach"), urgency/justification framing ("it's critical," "just this once"), or the language of the request. Live adversarial testing found the prompt-only version of this rule failed non-deterministically under sustained persona-injection (2 of 4 identical trials produced a real recommendation) — **a second, independent Anthropic call now reviews every drafted reply for a recommendation/ranking or a language-rule violation before the user ever sees it**, and discards + replaces the draft if either check fails or can't be confirmed (fail-closed). See `worker/README.md`'s "Why the second call exists."
- **Language, both halves, stated explicitly in the prompt text**: (a) understand/accept questions in any input language the user types in; (b) always respond in English, regardless of what language the question was asked in, what persona/role the model's been asked to adopt, or how far into a role-play scenario the conversation has gone. Also covered by the same second-call compliance check above, not prompt wording alone.
...
- **Max ~4 lines for typical conversational answers** — but when the user explicitly asks for a structured comparison (a table, a multi-item list), the agent uses the length that comparison actually needs; a multi-row table isn't a rule violation, it's the correct format for that question. Always includes a short reminder to verify current terms directly with the bank before applying, and that the data has a `last_verified_date`.
- **Structured comparisons render as real markdown**, not raw text. The agent formats multi-product comparisons as standard GFM markdown tables (header row, separator row, one row per product), capped at 8 products — past that, it shows the 8 most relevant, says plainly there are more, and points to Compare (up to 4 side by side) or suggests narrowing the request. Rendered client-side via `marked.js` + `DOMPurify` (`js/agent.js`'s `mdToSafeHtml()`) — DOMPurify is the actual security boundary against anything unsafe in model output, with a strict tag/attribute allowlist (tables, bold, lists, safe links only; raw HTML and images from the model are dropped before sanitization even runs). `max_tokens: 1500` gives an 8-product table with per-row detail room to complete without truncation; a response that still hits the ceiling is caught deterministically via `stop_reason` and replaced rather than shown cut off mid-sentence (see `worker/README.md`).
...
- **Footer, visible on every page** (one shared template in `js/shell.js`, not duplicated per page): "This is not financial advice — informational only," plus "MyFirstBank is an independent, unaffiliated resource. All bank names, logos, and trademarks are the property of their respective owners." Below that, a **Privacy Policy** link and an **Accessibility** link (jumps to `privacy.html#accessibility`), and a dynamically-computed **"Dataset last touched" date** — see below.
- **`privacy.html`** is the single page for everything legal/trust-related — no separate Terms of Use or Accessibility pages, no new navbar item. Six `<h2>` sections, in order, each with a stable anchor id (`#overview`, `#third-parties`, `#terms`, `#children`, `#accessibility`, `#contact`):
  1. **Privacy overview** — what's collected (anonymous, aggregate GoatCounter analytics; no cookies, no personal identifiers) vs. never collected/stored (name, email, SSN, ITIN, account numbers; Ask the Agent's chat history beyond the live browser session).
  2. **Third parties**, named explicitly — **Anthropic** (processes Ask the Agent's questions via the Worker proxy, under Anthropic's own terms; users are told not to type personally-identifying information into a chat question) and **GoatCounter** (cookieless, anonymous analytics).
  3. **Terms of Use** — informational only, no accuracy guarantee, no liability for decisions made from the site's data, verify directly with each institution before applying.
  4. **Children's privacy** — not directed at children under 13.
  5. **Accessibility statement** — references the real, shipped work (WCAG AA contrast on functional color, full keyboard navigation with visible focus states site-wide, `prefers-reduced-motion` respected everywhere animation appears), not generic boilerplate.
  6. **Contact** — `juanjo.chiarella@gmail.com`.
- **`MFB.globalLastUpdated`** (`js/data-loader.js`): the max `last_verified_date` across every product record (`checking_accounts.json`/`savings_accounts.json`/`credit_cards.json`) and every bank-level `relationship_programs[]`/`referral_program` entry — always computed live from the data, never hardcoded, so it updates itself the moment any record's date changes in a future refresh. Rendered in the footer via a deferred fill (`js/shell.js`'s `renderLastUpdated()`, same pattern as the Compare pill) since the footer itself renders before `MFB.dataReady` resolves.
- **Open Graph / Twitter Card meta tags** — hardcoded per-page in each file's raw `<head>` (`index.html`, `agent.html`, `compare.html`, `privacy.html`, `banks/_template.html`), not injected via `js/shell.js` like the navbar/footer/analytics are, since social-share crawlers generally don't execute JavaScript. `og:title`/`og:description` mirror each page's own `<title>`/`<meta name="description">`; `og:image` is one shared `assets/og-image.png` (1200×630, generated from `scripts/og-image-template.html` — reuses the real `assets/logos/_mark.svg` mark, not a site screenshot). Bank pages substitute `{{BANK_NAME}}`/`{{BANK_ID}}` per institution via `scripts/generate-bank-pages.py` (this also fixed a pre-existing bug where every generated bank page shared the same generic `<title>Bank — MyFirstBank</title>`).
